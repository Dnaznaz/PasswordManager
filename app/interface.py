...
def get_password(**kwargs):
    ...
    if decPass is None or decName is None:
        msg = 'STATUS=ERROR MSG="could not retrive password with ID {0}"'.format(pID)
    else:
        msg = "STATUS=OK NAME={name} PASSWORD={password}".format(name=decName.decode(), password=decPass.decode())

    if TEST_MODE is True:
        print(msg)

    comm.responed(reqID, msg)
...
def set_password(**kwargs):
    ...
    if crypto.authorize(auth_password) is True:
        res = dm.set_in_database(pID, encName, encPass, iv)

    if res == -1:
        msg = 'STATUS=ERROR MSG="could not add or update password {0}"'.format(name)
    else:
        msg = 'STATUS=OK ID={pID}'.format(pID=res)
...
def delete_password(**kwargs):
    ...
    if res is True:
        ...
        msg = 'STATUS=ERROR MSG="could not delete password with id {pID}"'.format(pID=res)
...
def get_data_list(**kwargs):
    ...
    for pID, name in nameDict.items():
        res += " SET=(ID={pID}:NAME={name})".format(pID=pID, name=name.decode())
...
def set_user(**kwargs):
    ...
    reqID = _pop_value(kwargs, "REQ_ID")
    auth_password = _pop_value(kwargs, 'AUTH_PASSWORD')
    old_pass = _pop_value(kwargs, "OLD_PASS")
    ...
    if crypto.authorize(old_pass) is False:
        print('old pass incorrect')
        return
    ...
    data['default']['salt'] = salt.decode()

    with open('config/config.json', 'w') as data_file:
        json.dump(data, data_file)

    crypto.no_owner = False
    crypto.MASTER_PASSWORD = hashedPass
    crypto.SALT = salt.decode()
